...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        # keep track of which locations are uncovered
        self.dug = set()
    ...
    def assign_values_to_board(self):
        for r in range(self.dim_size):
            for c in range(self.dim_size):
                if self.board[r][c] == "*":
                    # if already a bomb don't calculate anything
                    continue
                self.board[r][c] = self.get_num_neighboring_bombs(r, c)

    def get_num_neighboring_bombs(self, row, col):
        # make suer to not go out of bounds
    ...
    def dig(self, row, col):
        # true if successful dig, false if game over
    ...
    def __str__(self):
        # create a new array of what user sees
        visible_board = [
...
def play(dim_size=10, num_bombs=10):
    # step 1: create board and play bombs
    board = Board(dim_size, num_bombs)
    # step 2: show the user the board and ask where they want to dig

    # step 3a: if location is a bomb, show game over
    # step 3b: if location is not a bomb, dig recursively until each square is next to bomb
